 ...
 * トークンからテナントIDを取得
 */
export async function getTenantIdFromToken(token: string): Promise<string | null> {
  const payload = await verifyToken(token);
 ...
 * トークンからロールを取得
 */
export async function getRolesFromToken(token: string): Promise<JWTPayload['roles'] | null> {
  const payload = await verifyToken(token);
