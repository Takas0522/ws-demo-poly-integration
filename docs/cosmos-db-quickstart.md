...
**⚠️ 重要**: 初回起動時、CosmosDBエミュレーターの起動に2-3分かかります。起動完了まで待ってから次のステップに進んでください。
...
**注意**: Linux版CosmosDBエミュレーターではWebベースのData Explorerは利用できません。

データの確認には以下の方法を使用：
- Pythonスクリプトでの直接アクセス
- Azure Storage Explorerなどのツール
- アプリケーションのAPIエンドポイント経由
