...
#### 3.1.4 ロール管理機能（タスク04で実装完了）

Phase 1のタスク04により、完全なロール管理機能が実装されました：

**実装された機能**:
- ✅ **ロール割り当てAPI**: 全体管理者がユーザーにロールを割り当て可能
- ✅ **ロール削除API**: 全体管理者がユーザーからロールを削除可能
- ✅ **ユーザーロール一覧API**: ユーザーが持つロールの一覧を取得可能
- ✅ **利用可能ロール一覧API**: テナントが利用可能な全ロールを取得可能
- ✅ **JWT統合**: JWT生成時にユーザーのロール情報を含める
- ✅ **認可チェック**: `require_role`デコレータによるロールベース認可が完全に有効化

**セキュリティ保証**:
- 重複割り当て防止（同じロールを重複して割り当てない）
- テナント分離強制（他テナントのユーザーにロールを割り当てない）
- 監査ログ記録（すべてのロール操作を記録）
- 権限チェック（全体管理者のみがロール操作可能）
...
**データ整合性**:
- ユーザー削除時、RoleAssignmentも自動削除（カスケード削除）
- 決定的IDによる重複防止（`ra_{user_id}_{service_id}_{role_name}`）
...
| 1.4.0 | 2026-02-01 | ロール管理機能の実装完了を反映（タスク04対応）、ロール割り当て・削除API、JWT内ロール情報、認可チェックの有効化、データ整合性保証を追加 | [04-認証認可サービス-ロール管理](../../管理アプリ/Phase1-MVP開発/Specs/04-認証認可サービス-ロール管理.md) |
| 1.5.0 | 2026-02-01 | タスク05対応、特権テナント保護の詳細化、テナント削除時のユーザー数チェック、監査ログ記録の追加 | [05-テナント管理サービス-コアAPI](../../管理アプリ/Phase1-MVP開発/Specs/05-テナント管理サービス-コアAPI.md) |
| 1.6.0 | 2026-02-01 | タスク06対応、サービス間認証（API Key）の使用、DNS検証のセキュリティ考慮事項を追加 | [06-テナント管理サービス-ユーザー・ドメイン管理](../../管理アプリ/Phase1-MVP開発/Specs/06-テナント管理サービス-ユーザー・ドメイン管理.md) |
