...
class ParsedPermission(BaseModel):
    ...
    app: Optional[str] = Field(None, description="Application/service name (optional first segment)")
    ...
    wildcard_prefix: Optional[str] = Field(None, description="The wildcard level (e.g., 'app.*', 'app.users.*')")
...
class Role(BaseModel):
    ...
    permissions: List[str] = Field(default_factory=list, description="Permissions granted to this role")
    inherits_from: Optional[List[str]] = Field(None, description="Parent roles to inherit permissions from")
...
class UserPermissionContext(BaseModel):
    ...
    resource_owner_id: Optional[str] = Field(None, description="Resource owner ID (for 'own' scope checks)")
...
class PermissionCheckResult(BaseModel):
    ...
    matched_permission: Optional[str] = Field(None, description="Matching permission that granted access")
