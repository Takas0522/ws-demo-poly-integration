...
Dockerが使用できる場合はDevContainerでコードを開き、Codespacesが使用できる場合はそこからコードを参照してみます。
`day2` ブランチに切り替えて実施します。
Coding Agentの体験を行いたい場合は `Use This Template` からこのリポジトリを自分のアカウントにコピーします。
動作などさせたい場合は開発コンテナを起動してください。

## 1. 現時点のコードや機能の理解

コードやドキュメントを参照し、現時点でのコードの構成や機能を理解します。
Agentモードで下記を質問します。

``` markdown
開発中の機能が記載されているディレクトリを特定し
現在PoCアプリで計画されている開発について、開発中の機能の概要を教えてください。
```
...
- 開発タスクは開発者が開発しやすくかつテストやレビューなどの作業も実施しやすい粒度で構成されます
  - 各Issueの設計(デザイン)～開発までの一連の流れは一人の開発者でまとめて作業されます。レビューも工程に含まれます。
  - IssueのSuccess Criteriaはそれに準じて設定してください
- 開発タスク同士の依存関係や同時実行可能なタスクが存在する場合は明記します
- 参照すべきドキュメントがある場合は明記します。`docs/`からのパスで記述してください。
- 開発タスクごとにファイルが作成されます
  - 開発タスクの順番が明確にわかるようにタイトルに順番が記載されます
- 開発タスクのサマリレポートを作成します。並行実行可能、クリティカルパスなどの開発タスクを進めるうえでの参考情報が記載されます
- 内部で記載される情報として図示したほうが適切な場合はMermaid記法で記述されます
- 複数のリポジトリから構成されているため、更新対象となるリポジトリを文中に明記してください
  - 現在のメインリポジトリとサブモジュールとして定義されているリポジトリのみが対象です
- GitHub Issueを作成する元データとしても利用されるため、Issueに記載される内容はIssueのテンプレートに準じた内容で記載してください
```
...
```markdown
`docs/PoCアプリ/Specs/{追加機能名}/開発タスク/{Issueファイル名}`に記載された内容をもとに開発を行ってください。
```
...
```markdown
ユニットテストを実装することに特化したカスタムエージェントを構築します。
下記のように動作します。

- 実装された機能をもとにユニットテストの計画を構築し実装を行います
  - バックエンドはPyTest、フロントエンドはJestを使用してテストが構成されます
- 構築するのはテストなので実際に動作するアプリ側の実装を編集することは厳禁です
- テストは各サービスで展開されているリポジトリ内で実装される必要があります
- テスト実行後はテストレポートを作成し `docs/{アプリ名}/{開発プラン名}/{テスト日時}-ユニットテストレポート.md`として格納します
- e2eテストは対象外とします
<!-- 以下好きに構成 -->
- テストはISTQBで定義されているテスト要求に従います
- テストの実装後はテストを実施し失敗がなくなるまで実装を繰り返します
- テストの実施後、有効なテストとなっているかレビューを行い有効なテストとなっていない場合は警告を行います

カスタムエージェントの構築は下記のプラクティスなどを参考に日本語で構築してください

#fetch https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-custom-agents?utm_source=changelog-docs-custom-agents&utm_medium=changelog&utm_campaign=universe25
#fetch  https://github.blog/ai-and-ml/github-copilot/how-to-write-a-great-agents-md-lessons-from-over-2500-repositories/
```
...
精度が出ない場合ちょっとサブエージェントを使って作業を分離して改善します。

```markdown
現在の構成をオーケストレーターとサブエージェントの構成に変更します。
下記のように動作するように変更して下さい。

1. 対象のテストのユニットテストのテストシナリオを作成します
2. 下記の観点でテストシナリオをレビューします。レビュー結果はテストシナリオに反映します
   - テストの観点が十分であるか
   - テストの観点が重複していないか
   - テストの観点がテスト対象の機能に対して適切であるか
3. テストの実装
   - 実装のみでテストの実施は行いません
4. テストのレビュー
   - 有効なテストになっているかどうかレビューを行い問題がある場合は修正します
   - 絶対にパスするような状態になっていたり、逆に常に失敗するような状態になっている場合は警告を行い修正します
5. テストの実施
    - テストを実施し失敗がなくなるまで実装を繰り返します 

サブエージェントの構成については下記を参考にしてください
...
1. 開発タスクを作成します
   ...
   - 開発タスクはそれぞれ完了条件が存在しエージェントの作業内で確認可能な粒度で作成されます
     - 各タスク: `docs/PoCアプリ/Specs/{追加機能名}/開発タスク/{Issueファイル名}`
   - 開発タスクの進捗を管理するための開発タスクリストが作成されます。このエージェントで作業される単位で開発タスクリストは作成されます。
     - 進捗管理: `docs/PoCアプリ/Specs/{追加機能名}/開発タスク/開発タスク.md`
...
フロントエンドのテストはすでに構築されたカスタムエージェントがあるのでそれを利用します。
